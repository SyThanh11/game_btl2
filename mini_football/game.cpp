...
void TinyFootball::hit_ball(int type, ball_type type_of_ball)
{
...
void TinyFootball::behav_ball(ball_type type_of_ball)
{
	piece& ball = _pieces[tball];
	...
	hit_ball(1, type_of_ball);
	hit_ball(2, type_of_ball);
	hit_ball(3, type_of_ball);
	hit_ball(4, type_of_ball);
	hit_ball(5, type_of_ball);
	hit_ball(6, type_of_ball);
	...
	while (!quit)
	{
		...
		while (!quit) 
		{
			...
			if (_pvp)
			{
				...
				{
					countTus = 1;
					if (type_of_ball == normal) {
						type_of_ball = ice;
					}
					else type_of_ball = normal;
					std::cout << "doi state \n";
				}
				_lib->draw_ball(type_of_ball);
				behav_ball(type_of_ball);
				_lib->draw(_pieces);
			}
			else if (_pvc)
			{
